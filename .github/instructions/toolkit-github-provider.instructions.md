...
- The constructor reads `GITHUB_TOKEN` automatically and raises `ValueError` immediately if no token is available.
- The underlying transport hits `https://api.github.com/graphql`. Rate limits (5,000 points/hour) and GraphQL errors are raised as Python exceptions.
...
When you need deterministic cleanup, use the provider as a context manager:
...
| `fetch_commits(...)` | `defaultBranchRef.target.history` limited to 100 commits | `message`, `committedDate`, `additions`, `deletions`, `changedFiles`, `author` |
...
- Each method wraps `client.execute` and raises `Exception` with the underlying GraphQL message.
- For long-running agents, instantiate the provider once and reuse it.
- GraphQL schemas are fetched on first transport use. When developing offline, consider setting `fetch_schema_from_transport=False`.

## Next Steps

- [Social Media Tools](./toolkit-social-media.instructions.md) - Communication channels
- [Memory Tools](./toolkit-memory-mem0.instructions.md) - Long-term memory
