...
- Returns native balances (Ether-equivalent) when `token_address` is omitted, or ERC-20 balances converted using on-chain decimals.
- Outputs include only the formatted balance value (no decimals field).
...
- [Neo Tools](./toolkit-crypto-neo.instructions.md) - Neo blockchain integration
